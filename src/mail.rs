...
/// sample stuff - oauth login, change to real client ids later
pub fn oauth_login(client_id: &str, client_secret: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    // start server first so it's listening when browser connects
    // bind to all interfaces to avoid localhost binding issues
    let server = Server::http("0.0.0.0:8080")?;

    // print auth url (in case open::that fails) and try to open browser
    println!("Open this URL if browser didn't open:\n{}", auth_url.as_str());
    let _ = open::that(auth_url.as_str());

    // wait for redirect with code
    let request = server.recv()?; // blocks until browser hits the redirect
